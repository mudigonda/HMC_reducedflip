...
h_feval = figure(111);
if nargin < 4
    plot([fevalsstandard(2:end,1),fevalsstandard_persist(2:end,1),'r',...
    fevalsreduced_flip(2:end,1)],'g',...
    [fevalsstandard(2:end,2),fevalsstandard_persist(2:end,2),'b',...
    ...
    plot(fevalsstandard(2:end,1),fevalsstandard(2:end,2),'r',...
        fevalsstandard_persist(2:end,1),fevalsstandard_persist(2:end,2),'g',...
    fevalsreduced_flip(2:end,1),fevalsreduced_flip(2:end,2),'b',...
    feval_2_momentum(2:end,1),feval_2_momentum(2:end,2),'y');       
    legend('standard','standard, persistent mom','reduced flip','two momentum');
    xlabel('function evaluations');
    ylabel('percent error diff');
    title('percent error diff of model and sample covariances - diagonoal elements only');
